 ...
 * Engine.java
 * 
 * Created by Davis Herring 
 * Updated by Jeremy Davis (May 28, 2008)
 * 
 */
...
public class Engine extends Thread implements PoolController {

	private SAI eai; 
	private final Waves applet; 
	private final WidgetsPanel widgetsPanel; //Must implement this class later [JD]
	private final Controls controls; //Must implement this class later [JD]
	
	
	//WApplication Objects
	public final StatusBar statusBar; //Make visible so that other classes can call this as well [JD]
	...
	private SFInt32 widgets_setChoice;
	...
	//Create an Iterator class for the widgets list
	private class WIterator{
		...
		//Must implement PoolWidget class next [JD]
	...
	private float[] sparseHeight, height;
	
	private static float[] square(int size) {return new float[size*size];}
	
	private Pool pool; //Also must implement this class [JD]
...
//	public for WidgetsPanel's sake; blah [Davis]
	public int linearCounter, radialCounter, sampleCounter, pluckedCounter, struckCounter;
	...
	public Engine(Waves w){
		super("Waves Simulation Animation Thread #" + nextThreadID());
		//DEBUG
		...
		widgets_setChoice = (SFInt32)w.getSAI().getField("Widget-SWITCH", "whichChoice");
		
		sources = new Vector();
		widgets = new Vector();
		singleSources = new Vector();
		
		System.out.println("Engine is creating a new pool");
		pool = new Pool(w.getSAI(), POOL_SIZE, applet); //[JD]
		resolution = pool.getResolution(); //[JD]
		spacing = pool.getSpacing(); //[JD]
		sparseResolution = pool.getSparseResolution(); //[JD]
		sparseSpacing = pool.getSparseSpacing(); //[JD]
		
		height = square(resolution);
		sparseHeight = square(sparseResolution);
		
		widgetsPanel.setAutoSelect(false); //[JD]
	...
	//Callilng this method overrides the WApplication getStatusBar() method...
	//Made the StatusBar Object "statusBar" public in Waves.java...just call that instead... [JD]
	//public StatusBar getStatusBar() {return statusBar;}
	
	private boolean isSparse() { 
		return pool.getRenderingMode() == Pool.SPARSE;
	}
	
	public float getHeight(float x, float y){
		return getHeight(x,y,t);
	}
	...
	private float getHeight(float x, float y, float t){
		...
		for(Enumeration waveSources = sources.elements(); waveSources.hasMoreElements();)
			h += ((WaveSource) waveSources.nextElement()).getValue(x,y,t); //must implement WaveSource [JD]
		
		for(Enumeration singleWaveSources = singleSources.elements(); singleWaveSources.hasMoreElements();)
			h += ((SingleWaveSource) singleWaveSources.nextElement()).getValue(x,y,t); //must implement [JD]
	...
	public void update(){
		int i,p = 0;
		int u, v, uvmax;
		float x,y, space, newHeight[];
		boolean lores = isSparse();
		
		//uncomment for sampling stick
		/*if(sampleCount > 0){
			WIterator w = new WIterator();
			i = 0;
			do{
				PoolWidget pw = w.next();
				if(pw instanceof SamplingStick){ //Must implement SamplingStick [JD]
					++i;
		...
		//height2 and sparseHeight2 disabled for now [JD]
		...
		try{
			if(lores){
				...
				if(!playing)statusBar.setText(""); //may need to make this null [JD]
			}
	...
	//Animation and threaded methods 
	public void run(){
		long time;
		playing = false; 
		done = false; 
		...
		while(!done){
			...
			try{
				...
				if(time < minUpdateInterval)
					time = minUpdateInterval;
				sleep(time);
				synchronized(this) {
					betweenFrames = true; 
	...
	public void play(){
		...
		wasPlaying = playing; 
		playing = true;
		applet.getControls().setPlaying(true); //implement in Controls.java [JD]
	...
	public void pause(){
		wasPlaying = playing; 
		playing = false; 
		applet.getControls().setPlaying(false);
	}
	...
	//End animation and threaded methods [JD]
	...
	//Commenting this method out for now...WSLPlayer is different now [JD]
	...
	//Implement PoolController.java [JD]
	...
	//not known if this method is needed, but here it goes anyway [JD]
	public SAI getSAI() { return applet.getSAI(); }
	
	//commented out for now, here just in case [JD]
	/* public WSLPlayer getWSLPlayer() { return wslPlayer; } */
	
	//Must implement LinearSource [JD]
	public LinearSource addSource(float amplitude, float wavelength, float phase, float angle){
		System.out.println("Attempting to add source");
		float x, y, widgetSpacing;
		int i;
		...
		if(linearCount == 1) 
			controls.setLayoutButtonsEnabled(true);
		arrangeLinearWidgets();
		widgetsPanel.addSource(s);
		
		if(!playing && autoUpdate)
			update();
		
		//This line could conflict with WidgetsPanel's setAutoSelect() method, 
		//but it seems to be necessary to make the source's widgets display when the source is added.
		//widgetsPanel.show(s);
		return s;
	}
	
	//Uncomment for PluckedSource
	/*public PluckedSource addSource(float amplitude, float width, float xy[]){
		...
		//if(wslPlayer.isPlaying()) ps.setEnabled(false); //[JD]
	...
	/*public StruckSource addStruckSource(float amplitude, float width, float xy[]){
		...
		//if(wslPlayer.isPlaying()) ss.setEnabled(false); //[JD]
	...
	/*public RadialSource addSource(float amplitude, float wavelength, float phase, float x, float y){
		...
		//if(wslPlayer.isPlaying()) s.setEnabled(false); //[JD]
		sources.addElement(s);
		widgets.addElement(s);
		widgetsPanel.addSource(s);
	...
	public boolean removeWidget(PoolWidget s){
		if(widgets.contains(s)){
			...
			if(s instanceof LinearSource){
				linearCount--;
				if(linearCount == 0)
					controls.setLayoutButtonsEnabled(false);
				else
					arrangeLinearWidgets();
			}
			//Uncomment for RadialSource/SamplingStick
			/*
			...
			*/
				...
				System.err.println("Engine::removeWidget: Unexpected Widget " + s);
	...
	public PoolWidget getWidget(int index){
		if(index>0 && index<widgets.size())
			return ((PoolWidget) widgets.elementAt(index));
		else{
			System.out.println("getWidget::returning null");
			return null;
		}
	}
	...
	public void reset(){
		...
		//The default existing point sources [JD]
		//Make some default LinearSources to add here and uncomment these 2 lines when PointSource is 
		//implemented
		/*
	...
	//Ended with the WSL Methods..may not be needed, but put them in anyway commented out
	//Scripting is going to be a killer :( [JD]
